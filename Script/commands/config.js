...
try {
  ...
  cookie = appState.map(item => item = item.key + "=" + item.value).join(";");
} catch (_) {
  try {
    const fromEnv = require("../../includes/login/appState").loadAppStateFromEnv();
    if (fromEnv) cookie = fromEnv.map(item => item.key + "=" + item.value).join(";");
  } catch (__) {}
}
const headers = {
